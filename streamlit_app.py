...
streamlit.header("View our fruit list & Add ur favorites!!")
#snowflake functions:
...
def insert_fruit_snowflake(new_fruit):
  with my_cnx.cursor() as my_crsr:
    my_crsr.execute("insert into fruit_load_list values ('"+new_fruit+"')")
    return "Thanks for adding: "+new_fruit
    
usr_fruit=streamlit.text_input('What fruit would you like to add in this list?')
if streamlit.button('Add a fruit to the list?'):
  my_cnx= snowflake.connector.connect(**streamlit.secrets["snowflake"])
  my_data=insert_fruit_snowflake(usr_fruit)
  my_cnx.close()
